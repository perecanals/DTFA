...
def draw_roc_train_validation(preprocessed_data_list, summary_list, output_dir=None):
    fpr_train_list = []
    tpr_train_list = []
    fpr_test_list = []
    tpr_test_list = []
    ...
    for idx, summary in enumerate(summary_list):
        ...
        y_test_class_pred = summary["y_test_class_pred"]
        ...
        precision_train, recall_train, _ = precision_recall_curve(y_train_class, summary["y_train_reg_pred"])
        precision_train_list.append(precision_train)
        recall_train_list.append(recall_train)
        precision_test, recall_test, _ = precision_recall_curve(y_test_class, summary["y_test_reg_pred"])
        ...
        weighted_precision_list.append(precision_score(y_test_class, y_test_class_pred, average = "weighted"))
        weighted_recall_list.append(recall_score(y_test_class, y_test_class_pred, average = "weighted"))
        weighted_f1_score_list.append(f1_score(y_test_class, y_test_class_pred, average = "weighted"))
        ...
        cohens_kappa_list.append(cohen_kappa_score(y_test_class, y_test_class_pred))
        matthews_corrcoef_list.append(matthews_corrcoef(y_test_class, y_test_class_pred))
    ...
    tprs_upper_test = np.minimum(mean_tpr_test + 1 * std_tpr_test, 1)
    tprs_lower_test = np.maximum(mean_tpr_test - 1 * std_tpr_test, 0)

    # Plot the ROC curve
    plt.figure(figsize=(5, 5), dpi = 100)
    plt.rcParams.update({'font.size': 14})
    plt.plot(mean_fpr_train, mean_tpr_train, color="b", lw=2, label=f"Train ROC (AUC = {np.mean(roc_auc_train_list):.2f} $\pm$ {np.std(roc_auc_train_list):.2f})")
    plt.fill_between(mean_fpr_train, tprs_lower_train, tprs_upper_train, color="b", alpha=0.2)
    plt.plot(mean_fpr_test, mean_tpr_test, color="r", lw=2,  label=f"Test ROC (AUC = {np.mean(roc_auc_test_list):.2f} $\pm$ {np.std(roc_auc_test_list):.2f})")
    ...
    precisions_upper_test = np.minimum(mean_precision_test + 1 * std_precision_test, 1)
    precisions_lower_test = np.maximum(mean_precision_test - 1 * std_precision_test, 0)

    # Plot PR curve
    plt.figure(figsize=(5, 5), dpi = 100)
    plt.rcParams.update({'font.size': 14})
    plt.plot(mean_recall_train, mean_precision_train, color="b", lw=2, label=f"Train PR (AUC = {np.mean(pr_auc_train_list):.2f} $\pm$ {np.std(pr_auc_train_list):.2f})")
    plt.fill_between(mean_recall_train, precisions_lower_train, precisions_upper_train, color="b", alpha=0.2)
    plt.plot(mean_recall_test, mean_precision_test, color="r", lw=2,  label=f"Test PR (AUC = {np.mean(pr_auc_test_list):.2f} $\pm$ {np.std(pr_auc_test_list):.2f})")
    ...
    plt.ylabel('True DTFA')
    plt.xlabel('Predicted DTFA')
